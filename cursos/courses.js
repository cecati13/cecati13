...
class AvailableCourses {
  constructor(nameSpeciality) {
    ...
    if (courses.length > 0) {
      ...
      window.scroll(top);
      AvailableCourses.alternateTitle(
        `Cursos de ${nameSpeciality.toUpperCase()}`
      );
    }
  }
  ...
  constructorCourse(course) {
    ...
    course.imageURL =
      course.imagenURL != undefined ? URL_BASE_ASSETS + course.imagenURL : "";
    const container = document.createElement("div");
    const preregister = this.preRegistrationInformation(course);
    container.innerHTML = `
        <div class="course">
            <p class="containerCourse--title"><strong>${
              course.curso
            }</strong></p>
            ...
            <p>Horario:  <b>${course.hora_inicio} a ${
      course.hora_fin
    }</b> hrs.</p>
            <br>
            ...
            <textarea id="pre-${
              course.number
            }" style="display:none">${preregister}</textarea>
            <div  
    ...
    const informationTab = this.isTrueFichaInformacion(
      course.ficha_informacion
    );
    if (!informationTab) {
      courseInfo.remove();
    }
...
class ObjFromArray {
  ...
  sortBySpeciality(objCourses) {
    ...
    const forSpecialities = onlySpecialities.map((item) => {
      ...
      coursesArrayWithSpecialtie.forEach((course) =>
        imgArrayWithSpecialtie.push(course.imagenURL)
      );
...
class Specialties {
  static textTitle =
    "Selecciona una especialidad para ver los cursos disponibles:";

  constructor(arrayBySpecialties) {
    arrayBySpecialties.length > 0
  ...
  titleUpdate() {
    Specialties.textTitle =
      "Estamos actualizando nuestro sistema. Regresa más tarde para ver nuestros cursos disponibles.";
  }
  ...
  createContainer(arrayBySpecialties) {
    ...
    arrayBySpecialties.forEach((element) => {
      ...
      const imageRandom =
        Math.ceil(Math.random() * element.imageURL.length) - 1;
